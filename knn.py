...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            ...
            dists[i][j] = torch.sum(diff.pow(2))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(x_train.shape[0]):
        x_train_r = x_train[i].flatten().repeat((num_test, 1))
        diff = x_train_r - x_test.flatten(1, -1)
        dists[i] = torch.sum(diff.pow(2), dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    xr_f = x_train.flatten(1, -1)
    xs_f = x_test.flatten(1, -1)
    x_tr = xr_f.pow(2).sum(dim=1).repeat((num_test, 1)).T
    x_ts = xs_f.pow(2).sum(dim=1).repeat((num_train, 1))
    dists = x_tr + x_ts - xr_f.mm(xs_f.T) * 2
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        _, index = torch.topk(dists[:, i], k=k, largest=False)
        ts = y_train[index]
        unique_elements, counts = torch.unique(ts, return_counts=True)
        max_count_index = torch.argmax(counts)
        y_pred[i] = unique_elements[max_count_index]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    for item in k_choices:
        acc = []
        for i in range(num_folds):
            knn = KnnClassifier(torch.cat((x_train_folds[:i] + x_train_folds[i+1:]), dim=0), torch.cat((y_train_folds[:i] + y_train_folds[i+1:]), dim=0))
            accuracy = knn.check_accuracy(x_train_folds[i], y_train_folds[i], k = item)
            acc.append(accuracy)
        k_to_accuracies[item] = acc
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    # Replace "pass" statement with your code
    best_acc = 0.0
    for item, lst in k_to_accuracies.items():
        acc =  sum(lst) / len(lst)
        if  acc > best_acc:
            best_k = item
            best_acc = acc
